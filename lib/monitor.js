...
// Assuming a simple line of monitors. You can attach other monitor navigation methods
Monitor.prototype.goNext = function() {
  var cur_workspace = this.workspaces.current;
  this.go(cur_workspace+1);
  this.previous_workspace = cur_workspace; // @TODO: Check a config option before this line.
};

Monitor.prototype.goPrevious = function() {
  var cur_workspace = this.workspaces.current;
  this.go(cur_workspace-1);
  this.previous_workspace = cur_workspace; // @TODO: Check a config option before this line.
};
...
// Move a window to the previous workspace, and hide as necessary.
Monitor.prototype.windowPrev = Monitor.prototype.windowPrevious = function(id) {
  if ( this.nwm.windows.exists(id) ) {
    var window = this.nwm.windows.get(id);
    var cur_workspace = this.workspaces.current;
    var win_workspace = window.workspace;

    if ( win_workspace == cur_workspace ) {
      window.workspace = cur_workspace -1;
      window.hide();
    }
  }
};
...
// Move a window to the previous workspace, and hide as necessary.
Monitor.prototype.windowNext = function(id) {
  if ( this.nwm.windows.exists(id) ) {
    var window = this.nwm.windows.get(id);
    var cur_workspace = this.workspaces.current;
    var win_workspace = window.workspace;
