...
Monitor.prototype.rowColToIndex function rowColToIndex(row, col) {
  // row-major. 0-based.
  let idx = row * this.nwm.config.workspaces.cols + col;
...
Monitor.prototype.getWorkspaceToMy = function getWorkspaceToMy(direction) {
  let from          = this.workspaces.current,
      [row, col]    = this.indexToRowCol(from),
      to;
...
Monitor.prototype.go = function(workspace_id) {
  ...
  // always rearrange
  this.workspaces.get(monitor.workspaces.current).rearrange();
  return this;
};
